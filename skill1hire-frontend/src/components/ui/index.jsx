...
export function PageHdr({ title, sub, action }) {
  return (
    <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3 mb-6 sm:mb-8">
      <div>
        <h1 className="text-lg sm:text-xl font-black font-display" style={{ color: "var(--text)" }}>{title}</h1>
...
export function Tabs({ tabs, active, onChange }) {
  return (
    <div className="flex items-center gap-0.5 mb-6 overflow-x-auto scrollbar-hide -mx-4 px-4 sm:mx-0 sm:px-0" style={{ borderBottom: "1px solid var(--border)" }}>
      {tabs.map(t => (
        <button key={t.id} onClick={() => onChange(t.id)}
          className="px-3 sm:px-4 py-2.5 text-sm font-semibold border-b-2 -mb-px transition-all duration-150 rounded-t-lg whitespace-nowrap shrink-0"
          style={{ borderColor: active === t.id ? "var(--amber)" : "transparent", color: active === t.id ? "var(--amber)" : "var(--text-2)" }}>
          {t.label}
          {t.count !== undefined && (
            <span className="ml-1.5 sm:ml-2 text-[10px] font-bold px-1.5 py-0.5 rounded-full"
...
/* ─── Dashboard Skeleton ─────────────────────── */
export function DashSkeleton({ cards = 4, rows = 3 }) {
  return (
    <div>
      <Skel className="h-8 w-48 mb-2" />
      <Skel className="h-4 w-32 mb-6" />
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        {[...Array(cards)].map((_, i) => <Skel key={i} className="h-28" />)}
      </div>
      <Skel className="h-64" />
    </div>
  );
