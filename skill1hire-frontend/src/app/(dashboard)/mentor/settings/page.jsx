...
import { User, Mail, Lock, Calendar, ShieldCheck, Star, Briefcase, Clock, Sun, Moon, Trash2, Upload, X } from "lucide-react";
...
export default function MentorSettings() {
  ...
  useEffect(() => {
    mentorAPI.getProfile().then(({ data }) => {
      const p = data?.data?.profile;
      setProfile(p);
      reset({
        bio: p?.bio, currentRole: p?.currentRole, currentCompany: p?.currentCompany,
        yearsOfExperience: p?.yearsOfExperience, expertise: p?.expertise?.join(", "),
  ...
  const onSave = async (data) => {
    ...
    try {
      const payload = { ...data };
      if (data.expertise) payload.expertise = data.expertise.split(",").map(s => s.trim()).filter(Boolean);
  ...
  const handleAvatarUpload = async (e) => {
    ...
    try {
      ...
      await mentorAPI.updateProfile({ avatarUrl: result.url });
  ...
  return (
    <div className="max-w-4xl mx-auto pb-10">
      <PageHdr title="Settings" sub="Update your mentor profile and preferences" />

      <Card className="p-5 mb-8 bg-zinc-900/40 border-white/5 flex flex-col md:flex-row items-center gap-5">
        <Avatar src={profile?.avatarUrl || user?.profilePic} name={user?.name} className="w-20 h-20 border-4 border-white/5 ring-2 ring-purple-500/20" />
        <div className="flex-1 text-center md:text-left">
          <h2 className="font-bold text-white text-lg">{user?.name}</h2>
          {profile?.currentRole && <p className="text-sm text-zinc-400 mt-0.5">{profile.currentRole} @ {profile.currentCompany}</p>}
          <div className="flex items-center justify-center md:justify-start gap-2 mt-2">
            ...
            {profile?.avgRating > 0 && <Badge variant="amber" className="gap-1"><Star size={10} /> {profile.avgRating}</Badge>}
          </div>
        </div>
      </Card>
        ...
        <Card className="p-8 bg-zinc-900/20 border-white/5 shadow-xl">
          <form onSubmit={handleSubmit(onSave)} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <Field label="Current Role" placeholder="Senior Engineer" {...register("currentRole")} />
              <Field label="Current Company" placeholder="Google" {...register("currentCompany")} />
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <Field label="Years of Experience" type="number" placeholder="5" {...register("yearsOfExperience")} />
              <Field label="Avatar URL" placeholder="https://image-link.com" {...register("avatarUrl")} />
            </div>
            <Field label="Expertise (comma separated)" placeholder="React, Node.js, System Design" {...register("expertise")} />
            <Field label="LinkedIn URL" placeholder="linkedin.com/in/username" {...register("linkedinUrl")} />
            <TextArea label="Bio" placeholder="About your mentoring experience..." rows={4} {...register("bio")} />
      ...
      {tab === "upload" && (
        <Card className="p-8 bg-zinc-900/20 border-white/5">
          <div className="space-y-6">
            <p className="text-sm text-zinc-500 mb-4">Upload your profile photo.</p>
            ...
            {profile?.avatarUrl && (
              <div className="mt-6">
                ...
                <Avatar src={profile.avatarUrl} name={profile?.name || user?.name || "User"} size="xl" className="w-24 h-24" />
              </div>
            )}
          </div>
        </Card>
      ...
      {tab === "account" && (
        <Card className="p-8 bg-zinc-900/20 border-white/5">
          <div className="space-y-5">
            ...
            <AccountRow icon={ShieldCheck} label="Account Role" badge={<Badge variant="purple">Mentor</Badge>} verified={user?.isVerified} />
          ...
          <div className="mt-8 pt-8 border-t border-white/5">
            ...
            <div className="p-4 rounded-2xl bg-red-500/5 border border-red-500/10">
              ...
              <Btn 
                onClick={async () => {
                  if (confirm("Are you absolutely sure you want to delete your mentor account? This will lock your session immediately.")) {
                    try {
