...
export default function MentorDashboard() {
  ...
  if (loading) return <div><PageHdr title="Dashboard" /><div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">{[...Array(3)].map((_, i) => <Skel key={i} className="h-28" />)}</div></div>;
      ...
      <Card>
        ...
        {upcoming.length === 0 ? (
          ...
          <div className="divide-y" style={{ borderColor: "var(--border)" }}>
            {upcoming.map(s => (
              <div key={s._id} className="flex flex-col sm:flex-row sm:items-center justify-between px-4 sm:px-5 py-3.5 gap-3">
