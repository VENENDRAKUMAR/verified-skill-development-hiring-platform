...
export default function LoginPage() {
  ...
  return (
    <div>
      <h1 className="font-display italic text-4xl mb-2" style={{ color: "var(--text)" }}>Welcome back</h1>
      <p className="text-[15px] mb-8" style={{ color: "var(--text-2)" }}>Sign in to your Skill1 Hire account</p>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
        ...
        <div className="relative">
          ...
          <button type="button" onClick={() => setShow(!show)}
            className="absolute right-4 top-10 transition-colors hover:text-white" style={{ color: "var(--text-3)" }}>
            {show ? <EyeOff size={15} /> : <Eye size={15} />}
          </button>
        </div>

        <div className="flex justify-end">
          <Link href="/forgot-password" className="text-[13px] font-semibold hover:underline" style={{ color: "var(--amber)" }}>Forgot password?</Link>
        </div>
      ...
      <p className="mt-8 text-center text-[14.5px]" style={{ color: "var(--text-2)" }}>
        No account?{" "}
